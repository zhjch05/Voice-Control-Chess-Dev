...
Meteor.methods({
    ...
    addTheme: function(){
    console.dir(themeOfPiece.find().fetch());
console.log("test="+themeOfPiece.find().fetch().length==0);
var theSize = themeOfPiece.find().fetch().length;
if(theSize==0){
        console.log("adding theme"+theSize);
        pieceTheme="a";

        themeOfPiece.insert({pieceTheme:pieceTheme});
        console.dir(themeOfPiece.find().fetch());
}
console.log("after test"+JSON.stringify(themeOfPiece.find().fetch()));
    }
